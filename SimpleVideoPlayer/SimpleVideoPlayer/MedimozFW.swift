...
open class MedimozAVController : AVPlayerViewController {
    ...
    @objc private func interval() {
        ...
        let hearBeatInt = Int(self.heartBeat)
        let operatorDiff = currentTimeInt % hearBeatInt
        var informedInterval = 0;
        
        if (currentTimeInt >= hearBeatInt) {
            informedInterval = currentTimeInt
            if (operatorDiff != 0) {
                informedInterval = currentTimeInt - operatorDiff
            }
        }
        
        if (timeTracked.index(of: informedInterval) == nil) {
            timeTracked.append(informedInterval)
            self.matomoTracker.track(eventWithCategory: "video", action: "time-\(Int(informedInterval))", name: videoName, value: nil)
